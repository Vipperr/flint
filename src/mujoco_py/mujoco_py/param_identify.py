...
from ur5e_get_regression import ur5e_get_regression

# 读取数据
file_path = "/home/xiatenghui/work_space/mujoco_ws/src/mujoco_py/"
positions = pd.read_excel(file_path + "positions.xlsx")
velocities = pd.read_excel(file_path + "velocities.xlsx")
accelerations = pd.read_excel(file_path + "accelerations.xlsx")
efforts = pd.read_excel(file_path + "efforts.xlsx")
...
for q, dq, ddq, tau in zip(q_log, dq_log, ddq_filtered, tau_log):
    Y = np.array(ur5e_get_regression(q, dq, ddq))  # Y: dof x n_baseparms
    Y = Y.reshape(6, -1)
    Y_total.append(Y)
